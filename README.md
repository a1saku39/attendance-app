...
#### 社員マスタシート
1. スプレッドシートの「**社員マスタ**」シートに**D列、E列、F列**を追加します
2. 1行目に以下のヘッダーを入力:
   - D列: `承認対象部署`
   - E列: `第1承認者`
   - F列: `第2承認者`
3. 2行目以降に各部署の承認者を設定します
   - 承認権限を持つ社員の行に、その人が承認する部署と役割を記載します
   - または、承認設定専用の行を作成しても構いません
   ...
   ```
   ※この例では:
   - 営業部の承認: 第1承認者は「佐藤 花子」、第2承認者は「山田 太郎」
   - 総務部の承認: 第1承認者は「佐藤 花子」、第2承認者は「田中 次郎」
   - 承認を実行する際は、E列またはF列に入力された名前と完全に一致する名前を入力する必要があります。
...
3. **承認の実行**
   ...
   - 承認状況に応じて、以下のボタンが表示されます:
     - **第1承認を実行**: その部署の第1承認者として設定されている人のみ実行可能
     - **第2承認を実行**: 第1承認完了後、その部署の第2承認者として設定されている人のみ実行可能
   - ボタンをクリックして承認を実行
...
4. **承認フロー**
   ...
   - 社員マスタで承認者として設定されていない人は承認できません

### 注意事項
- 承認を実行する前に、必ず社員マスタのD〜F列が正しく設定されているか確認してください
